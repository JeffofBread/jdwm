...
void drw_resize(Drw *drw, unsigned int w, unsigned int h)
{
	...
	drw->picture =
		XRenderCreatePicture(drw->dpy, drw->drawable,
				     XRenderFindVisualFormat(drw->dpy, DefaultVisual(drw->dpy, drw->screen)), 0, NULL);
}
...
Picture drw_picture_create_resized(Drw *drw, char *src, unsigned int srcw, unsigned int srch, unsigned int dstw,
				   unsigned int dsth)
{
	Pixmap pm;
	Picture pic;
	...
	if (srcw <= (dstw << 1u) && srch <= (dsth << 1u)) {
		...
		pic = XRenderCreatePicture(drw->dpy, pm, XRenderFindStandardFormat(drw->dpy, PictStandardARGB32), 0,
					   NULL);
		 ...
		 int filled) /* wrapper function to scale and draw a polygon with X11 */
{
	...
	if (filled)
		XFillPolygon(
			drw->dpy, drw->drawable, drw->gc, scaledpoints, npoints, shape,
			CoordModeOrigin); /* Change shape to 'Convex' or 'Complex' in dwm.c if the shape is not 'Nonconvex' */
	else
