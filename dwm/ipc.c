 ...
 * @brief Part jdwm's IPC (Inter Process Communication) system, originally written by Mihir Lad as the ['dwm-ipc' patch](https://github.com/mihirlad55/dwm-ipc)
...
static int ipc_run_command(IPCClient *ipc_client, char *msg)
{
	...
	if (ipc_get_ipc_command(parsed_command.name, &ipc_command) < 0) {
		ipc_prepare_reply_failure(ipc_client, IPC_TYPE_RUN_COMMAND, "Command %s not found",
					  parsed_command.name);
