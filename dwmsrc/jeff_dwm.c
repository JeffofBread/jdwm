...
enum { SchemeNorm, SchemeSel, SchemeSystray, SchemeStatus, SchemeTagsSel, SchemeTagsNorm, SchemeInfoSel, SchemeInfoNorm }; /* color schemes */
...
struct ClientState {
	int isfixed, isfloating, isurgent, neverfocus, oldstate, isfullscreen, ignoretransient, beingmoved, issticky, fakefullscreen, issteam, hintsvalid;
};
...
setup(void)
{
    ...
    XChangeProperty(dpy, wmcheckwin, netatom[NetWMName], utf8string, 8,
        PropModeReplace, (unsigned char *) "jeff_dwm", 8);
...
main(int argc, char *argv[])
{
    ...
    if (!(dpy = XOpenDisplay(NULL)))
        die("jeff_dwm: cannot open display");
    checkotherwm();
    setup();
#ifdef __OpenBSD__
